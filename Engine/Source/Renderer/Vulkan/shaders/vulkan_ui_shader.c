...
b8 vulkan_ui_shader_create(vulkan_context* context, vulkan_ui_shader* out_shader) {
    ...
    for (u32 i = 0; i < UI_SHADER_STAGE_COUNT; ++i) {
        if (!create_shader_module(context, BUILTIN_SHADER_NAME_UI, stage_type_strs[i], stage_types[i], i, out_shader->stages)) {
            LOG_ERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_UI);
            return FALSE;
        }
    }
    ...
    memory_zero(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
...
#define ATTRIBUTE_COUNT 2
    ...
    if (!vulkan_buffer_create(
        ...
        LOG_ERROR("UI instance buffer creation failed for shader.");
        return FALSE;
    }
...
void vulkan_ui_shader_destroy(vulkan_context* context, struct vulkan_ui_shader* shader) {
    VkDevice handle = context->device.handle;

    vkDestroyDescriptorPool(handle, shader->object_descriptor_pool, context->allocator);
    vkDestroyDescriptorSetLayout(handle, shader->object_descriptor_set_layout, context->allocator);
    ...
    vkDestroyDescriptorPool(handle, shader->global_descriptor_pool, context->allocator);

    // Destroy descriptor set layouts.
    vkDestroyDescriptorSetLayout(handle, shader->global_descriptor_set_layout, context->allocator);
...
void vulkan_ui_shader_use(vulkan_context* context, struct vulkan_ui_shader* shader) {
    ...
    vulkan_pipeline_bind(&context->command_buffers.data[current_image], VK_PIPELINE_BIND_POINT_GRAPHICS, &shader->pipeline);
}
...
void vulkan_ui_shader_update_global_state(vulkan_context* context, struct vulkan_ui_shader* shader, f32 delta_time) {
    ...
    VkDescriptorSet global_descriptor = shader->global_descriptor_sets[current_image];
    ...
    descriptor_write.dstSet = shader->global_descriptor_sets[current_image];
...
void vulkan_ui_shader_apply_material(vulkan_context* context, struct vulkan_ui_shader* shader, Material* material) {
    if (context && shader) {
        ...
        memory_zero(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
        ...
        for (u32 sampler_index = 0; sampler_index < sampler_count; ++sampler_index) {
            ...
            u32* descriptor_generation = &object_state->descriptor_states[descriptor_index].generations[current_image];
            u32* descriptor_id = &object_state->descriptor_states[descriptor_index].ids[current_image];
...
void vulkan_ui_shader_release_resources(vulkan_context* context, struct vulkan_ui_shader* shader, Material* material) {
    ...
    VkResult result = vkFreeDescriptorSets(context->device.handle, shader->object_descriptor_pool, descriptor_set_count, instance_state->descriptor_sets);
    if (result != VK_SUCCESS) {
