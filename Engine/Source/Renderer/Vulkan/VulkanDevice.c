...
static b8 selectPhysicalDevice(VulkanContext* state);
...
b8 vulkanDeviceInit(VulkanContext* context)
{
    ...
    vkGetDeviceQueue(context->device.logicalDevice, context->device.queues.graphics.index, 0, &context->device.queues.graphics.handle);
    vkGetDeviceQueue(context->device.logicalDevice, context->device.queues.present.index, 0, &context->device.queues.present.handle);
    vkGetDeviceQueue(context->device.logicalDevice, context->device.queues.transfer.index, 0, &context->device.queues.transfer.handle);
...
void vulkanDeviceDestroy(VulkanContext* context)
{
    ...
    DARRAY_DESTROY(context->device.swapchainSupport.formats);
    DARRAY_DESTROY(context->device.swapchainSupport.modes);
}
    ...
    VulkanSwapchainSupportInfo* swapchainSupportInfo)
{
    ...
    if (surfaceFormatCount) {
        if (!swapchainSupportInfo->formats.capacity) {
            DARRAY_INIT(swapchainSupportInfo->formats, MEMORY_TAG_RENDERER);
            DARRAY_RESERVE(swapchainSupportInfo->formats, surfaceFormatCount);
        }
    ...
    if (presentModeCount) {
        if (!swapchainSupportInfo->modes.capacity) {
            ...
            DARRAY_RESERVE(swapchainSupportInfo->modes, presentModeCount);
        }
...
b8 selectPhysicalDevice(VulkanContext* context)
{
    ...
    for (u32 i = 0; i < physicalDevices.size; ++i) {
        ...
        if (result) {
            ...
            context->device.physicalDevice = DARRAY_AT(physicalDevices, i);
    ...
    VulkanSwapchainSupportInfo* swapchainSupportInfo)
{
    ...
    if (
        ...
        vulkanDeviceQuerySwapchainSupport(physicalDevice, surface, swapchainSupportInfo);
