...
b8 application_init(game_instance* game)
{
    if (game->app_state)
    {
        LOG_ERROR("application_init: Already called");
        return FALSE;
    }

    // Memory system must be the first thing to be stood up.
    memory_system_configuration memory_config = {};
    memory_config.total_memory_size_in_bytes = GIBIBYTES(1);
    if (!memory_system_startup(memory_config))
    {
        LOG_ERROR("application_init: Failed to initialize memory system");
        return FALSE;
    }

    game->app_state = memory_system_allocate(sizeof(*app_state), MEMORY_TAG_APPLICATION);
    ...
    if (!material_system_startup(&app_state->material_system.required_memory, app_state->material_system.memory, material_sys_config)) {
        LOG_FATAL("Failed to initialize material_resource system. Application cannot continue.");
        return FALSE;
    }
    ...
    geometry_system_configuration g_config = geometry_system_generate_plane_config(15.0f, 5.0f, 5, 5, 5.0f, 2.0f, "test geometry", "test_material");
